...
#define DEF_EVENT_ID( _TYPE_, _ID_ ) \
    ...
    public:
        ...
        uint32_t id() const { return m_msg.head().tid(); }
        ...
        const std::string& src() const { return m_msg.head().src(); }

        const std::string& dst() const{ return m_msg.head().dst(); }

        void set_src(const std::string& src) { m_msg.mutable_head()->set_src(src); }

        void set_dst(const std::string& dst) { m_msg.mutable_head()->set_dst(dst); }
