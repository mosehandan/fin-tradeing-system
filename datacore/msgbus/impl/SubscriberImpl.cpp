...
Subscriber* Subscriber::create(const char *addr, SubscriberCallback* cb) { return new SubscriberImpl(addr, cb); }
...
void SubscriberImpl::start() { m_thd = new thread(std::bind(&SubscriberImpl::run, this));
}
