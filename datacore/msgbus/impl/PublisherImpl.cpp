...
Publisher* Publisher::create(const char* addr) { return new PublisherImpl(addr); }

PublisherImpl::PublisherImpl(const char* addr) : m_thd(nullptr), m_should_exit(false)
{
...
PublisherImpl::~PublisherImpl() { delete m_sock; delete m_thd; }
...
void PublisherImpl::start() { m_thd = new std::thread(&PublisherImpl::run, std::ref(*this)); }

void PublisherImpl::stop() { m_should_exit = true; m_thd->join(); }
