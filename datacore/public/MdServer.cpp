...
namespace jzs {  
    ...
    bool MdServer::do_publish(jzs::msg::md::MarketDataType type, const jzs::msg::md::MarketQuote& quote, uint32_t jzcode) {
        MarketDataInd ind;
        ind.set_type(type);
        switch (type) {
    ...
    void MdServer::publish(jzs::msg::md::MarketDataType type, const jzs::msg::md::MarketQuote& quote_in)
    {
        // 保存时间戳最新的记录
        ...
        if (quote.mutable_qs()->date() != trade_day && quote.mutable_qs()->date() != action_day) {
